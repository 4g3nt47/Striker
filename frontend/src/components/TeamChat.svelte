  ...
  <div id="messages" class="border-2 border-green-700 p-1 no-scrollbar overflow-y-auto bg-gray-900 whitespace-pre-wrap break-all text-white">
    {#each messages as msg, index}
      {#if (msg.username && msg.date)}
        <span class="text-yellow-400">{formatDate(msg.date)}</span><span class="text-green-400">{msg.username.padStart(12, " ")}</span><span class="text-yellow-400">{ " > "}</span>{msg.message}
      {:else}
        <span class="text-green-400">{msg.message}</span>
      {/if}

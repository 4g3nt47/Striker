    ...
    <div class="main-body no-scrollbar w-full grid grid-cols-6">
      ...
      <div class="page-body col-span-5 no-scrollbar overflow-y-auto px-5 py-5 pt-10 bg-gray">
        ...
        {:else if (session.page === "agentPage" && selectedAgent !== null)}
          <AgentHandler {session} {socket} agent={selectedAgent} tasks={selectedAgentTasks} consoleMsgs={consoleMsgs[selectedAgent.uid]} on:clearConsole={clearConsoleHandler} on:updateConsole={updateConsoleHandler}/>
        {:else}
